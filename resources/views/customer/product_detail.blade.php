                        ...
                        <ul class="nav nav-tabs" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link active" data-toggle="tab"
                                    href="#description">{{ __('product.description') }}</a>
                            </li>
                            <li class="nav-item">
                        ...
                        <div class="tab-content">
                            ...
                            <div id="review" class="profile-tab tab-pane fade">
                                <div class="review-desc-text">
                                    <h6>{{ __('product.reviews') }} <sup
                                            class="green-text">({{ $product->ratings()->count() }})</sup></h6>
                                                ...
                                                <div class="product-rating">
                                                    <div class="star-ratings">
                                                        <div class="fill-ratings"
                                                            ...
                                                            <span class="d-flex"><i class="fas fa-star"></i><i
                    ...
                    <form class="filter-form" action="{{ route('book', [$product->id]) }}" method="get">
                            ...
                            <div class="product-detail-title">
                                ...
                                @if (@auth()->user()->role->name == 'customer')
                                    @if (!is_null($product->favorites))
                                        <div class="wishlist" onclick="addToWishlist(this, {{ $product->id }})"><span
                                                class="icon-box active"><i class="fas fa-heart"></i></span></div>
                                    @else
                                        <div class="wishlist" onclick="addToWishlist(this, {{ $product->id }})"><span
                                                class="icon-box"><i class="far fa-heart"></i></span></div>
                                    @endif
                                @endif
                            </div>

                            <p class="">Retailer : <a href="{{ route('retailer', $product->retailer->id) }}"><span
                                        class="blue-text ml-1">{{ $product->retailer->name }}</span></a></p>
                            @if (
                                !is_null(request()->get('reservation_date')) ||
                                    (!is_null(request()->get('location')) &&
                                        !is_null(request()->get('latitude')) &&
                                        !is_null(request()->get('longitude'))))
                                <div class="pro-booking-info">
                                        ...
                                        <p class="product-booking-info">
                                            <span>Reservation Date : </span><span class="grey-text ml-1">
                                                ...
                                                @if (count($reservationDate) == 2)
                                                    {{ $reservationDate[0] }} {{ $global_date_separator }}
                                                    {{ $reservationDate[1] }}
                                                @endif
                                            </span>
                                        </p>
                                        <input type="hidden" name="reservation_date" class="form-control"
                                    ...
                                    @if (
                                        ...
                                        <input type="hidden" name="latitude" class="form-control location-latitude"
                                            value="{{ request()->get('latitude') }}">
                                        <input type="hidden" name="longitude" class="form-control location-longitude"
                                            value="{{ request()->get('longitude') }}">
                                    @endif
                                    <span class="change-icon" data-toggle="modal" data-target="#bookProductModal"><i
                        ...
                        <div class="product-detail-price-review ">
                            <div class="product-detail-price">
                                <h5 class="mb-0 black-text">${{ $product->rent }} <small>/ @if ($product->rentaltype == 'Day')
                                            {{ __('product.day') }} @else{{ __('product.hour') }}
                                        @endif
                                    </small></h5>
                                <p class="smallFont black-text"><input type="radio" name="security_option"
                                        id="security" class="form-control security-option" value="security"
                                        checked><label for="security">{{ __('product.security') }}: ${{ $security }}
                                        <span class="grey-text">( {{ __('product.refundable') }} )</span></label><span
                                        class="icon-box small-icon ml-1" data-toggle="tooltip" data-placement="right"
                                        title="Security is refundable once you returned the product."><i
                                            class="fas fa-info"></i></span></p>
                                <p class="smallFont black-text"><input type="radio" name="security_option"
                                        id="insurance" class="form-control security-option" value="insurance"><label
                                        for="insurance">Insurance: ${{ $insurance }} <span class="grey-text">(
                                            non-refundable )</span></label><span class="icon-box small-icon ml-1"
                                        data-toggle="tooltip" data-placement="right"
                                        title="Insurance will cover your damage and it is non-refundable."><i
                                            class="fas fa-info"></i></span></p>
                                @error('option')
                            ...
                            <div class="product-rating ">
                                ...
                                <small><span>{{ $product->average_rating }} </span>({{ $product->ratings()->count() }}
                                    {{ __('product.reviews') }})</small>
                            </div>
                        </div>
                        ...
                        @else
                            <a href="javascript:void(0);" class="btn blue-btn large-btn fullwidth" data-toggle="modal"
                                data-target="#bookProductModal">{{ __('buttons.bookNow') }}</a>
                        @endif

                        <div class="product-location">
                            <p class="w-500 black-text largeFont">{{ __('product.productLocation') }} <span
                                    class="icon-box small-icon ml-1" data-toggle="tooltip" data-placement="right"
                                    title="Exact product location will be shown after booking has been done"><i
                                        class="fas fa-info"></i></span></p>
                        ...
                        <div class="product-card">
                            ...
                            <div class="product-reviews">
                                ...
                                <div class="product-rating ">
                                    <div class="star-ratings">
                                        <div class="fill-ratings"
                                            style="width: {{ $relatedProduct->average_rating * 20 }}%;">
                                            <span class="d-flex"><i class="fas fa-star"></i><i class="fas fa-star"></i><i
                                                    class="fas fa-star"></i><i class="fas fa-star"></i><i
                                                    class="fas fa-star"></i></span>
                                        </div>
                                    ...
                                    <small><span>{{ $relatedProduct->average_rating }}
                                        </span>({{ $relatedProduct->ratings()->count() }}
                                        {{ __('product.reviews') }})</small>
                                </div>
                            </div>
                            <a href="{{ sprintf($bookNow, $relatedProduct->id) }}"
                                class="btn blue-btn fullwidth blue-outline">Book Now</a>
                            @if (@auth()->user()->role->name == 'customer')
                                @if (!is_null($relatedProduct->favorites))
                                    <div class="wishlist" onclick="addToWishlist(this, {{ $relatedProduct->id }})"><span
                                            class="icon-box active"><i class="fas fa-heart"></i></span></div>
                                @else
                                    <div class="wishlist" onclick="addToWishlist(this, {{ $relatedProduct->id }})"><span
                                            class="icon-box"><i class="far fa-heart"></i></span></div>
...
@section('scripts')
    ...
    <div class="modal fade book-product-modal" id="bookProductModal" tabindex="-1" role="dialog"
        aria-labelledby="bookProductModalTitle" aria-hidden="true">
                                ...
                                <form class="filter-form" action="{{ route('book', [$product->id]) }}" method="get"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="location" id="location"
                                            class="form-control input-bg loc" value="{{ $selectedLocation }}"
                                            @if (!empty(request()->get('latitude'))) data-lat="{{ request()->get('latitude') }}" @endif
                                            @if (!empty(request()->get('longitude'))) data-lng="{{ request()->get('longitude') }}" @endif
                                            placeholder="{{ __('product.placeholders.location') }}">
                                        <input type="hidden" name="latitude" id="latitude"
                                            class="form-control input-bg loc location-latitude"
                                            value="{{ request()->get('latitude') }}">
                                        <input type="hidden" name="longitude" id="longitude"
                                            class="form-control input-bg loc location-longitude"
                                            value="{{ request()->get('longitude') }}">
                                        ...
                                        @endif
                                        <span class="location-prompt d-none underline" id="geolocate" role="button"><i
                                                class="fa fa-map-marker-alt"></i>Use my current location</span>
                                        <img src="{{ asset('img/location-spinner.svg') }}" id="locationLoader"
                                            class="d-none" alt="loader">
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="reservation_date"
                                            class="form-control modal-date-icon"
                                            @if ($product->rentaltype == 'Day') placeholder="{{ __('product.placeholders.reservationDate') }}" @else placeholder="{{ __('product.placeholders.reservationDateAndTime') }}" @endif
